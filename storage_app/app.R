...
ui <- fluidPage(
  theme = bslib::bs_theme(bootswatch = 'paper'),
  ...
  navbarPage(icon('world'),
    ...
    tabPanel('map',
             fluidRow(
               column(
                 ...
                 h3(textOutput('map_click_output')),
                 br(),
                 br(),
                 fillRow(flex = c(1,2),
                   height = 80,
                   actionButton('submit', 'Submit'),
                     ...
                     textInput(inputId = 'name', 'Name', placeholder = 'Your Name')
                   )
                 ),
                 br(),br(),
    ...
    tabPanel(
      ...
      tags$li('hello'),
      tags$li(
        'This app demonstrates among other things,
             persistent storage using a mysql database hosted on AWS'
      ),
      tags$li(
        'It leverages good practice in database communication and
             read write permissions'
      ),
      tags$li(
        'It utilises reactive programming paradigms to fully engage
             in server side rending in responsive to users interaction with the
             User Interface'
      ),
      tags$li("It also writes custom Javascript for the 'fly to' feature, extending DatatablesJS"),
      h3('TODO'),
      tags$li('Host on AWS EC2/lightsail/fargate'),
      tags$li(
        'Secure using aws best principles, VPC, subnets,
             internet gateways, Network access control lists'
      ),
      tags$li('Secure login entry routes usign AWS cognito'),
      tags$li('Use environ variables properly'),
      h3('Features'),
      tags$li('Introduce reverse geocoding')
    ),
    tabPanel('Quick docs',
             p('The initial instructions indicate that a name and a map click input are required'),
             p('Check also saved records by clicking on the that record row, or using the "fly to" in the far right column'),
             tags$img(src='to_start_edited.png',height='400px'),
             p('Then you can send to the database, persisiting these records witht a time stamp'),#,align='right'
             img(src='finish_edited.png',height='400px'),#,align='right'
             p('The database administration and maintenence component of the project was conducted in mysql workbench
               connecting to a managed rds on AWS'),
             img(src='mysql_editor.png',height='400px')
...
server <- function(input, output, session) {
  ...
  observeEvent(table_clicked(), {
    ...
    x <-
      rbind(current_table_contents())[input$table_rows_selected, ]
    print(x)
  ...
  output$map_click_output <- renderText({
    validate(need(input$map_click,
                  message = 'Click on the map below to start'))
    paste(
      'You have selected lat: ',
      round(input$map_click$lat, 3),
      '
        lng: ',
      round(input$map_click$lng, 3)
    )
  })
  ...
  output$table <- renderDT(server = T, {
    ...
    action <-
      DT::dataTableAjax(session = session,
                        data = df,
                        rownames = FALSE)
    print(action)
    #print(df)
